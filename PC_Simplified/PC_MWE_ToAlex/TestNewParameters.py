...
class MyModel_time(nn.Module):
    def __init__(self): 
        ...
        self.Jee = nn.Parameter(torch.tensor(-10.7245, requires_grad= True, dtype= torch.float64))#, requires_grad=False, dtype=torch.float32)#I replaced .072 by 0.072
        #ic(self.Jee.grad_fn) #should be none
        self.Jei =nn.Parameter(torch.tensor(37.0476, requires_grad= True, dtype= torch.float64)) 
        self.Jie = nn.Parameter(torch.tensor(65.4866, requires_grad=True, dtype=torch.float64))
        self.Jii = nn.Parameter(torch.tensor(0.2850, requires_grad=True, dtype=torch.float64))
        self.Jin = nn.Parameter(torch.tensor(-5518.0105, requires_grad= True, dtype=torch.float64))
    ...
    def phi(self, I_tot, a, b, hm): #)))  # this use a lot of memory - exponential part
        ...
        multi= torch.nan_to_num((torch.mul(a, I_tot) + b), nan = self.epsilon, posinf=9999999, neginf=-9999999)
