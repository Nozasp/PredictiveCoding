...
class MyModel_time(nn.Module):
    ...
    def phi(self, I_tot, a, b, hm): #)))  # this use a lot of memory - exponential part
        ...
        multi= torch.nan_to_num((torch.mul(a, I_tot) + b), nan = self.epsilon, posinf=9999999, neginf=self.epsilon)
        #multi= torch.nan_to_num((torch.mul(a, I_tot) + b), nan = .5, posinf=.5, neginf=.5)
        expo = torch.exp(- (multi)) #.abs()+ self.epsilon)
        #ic(expo.grad_fn)
        return torch.mul(hm, torch.divide(1, (1 + expo)))  #torch.multiply(hm, torch.divide(torch.ones(1), (torch.add(torch.ones(1), torch.exp(torch.add(torch.neg(torch.multiply(a, I_tot)), b)))
    ...
    def forward(self, In):
        ...
        s_ampa = torch.zeros(self.N)#torch.zeros(self.N)
        s_gaba = torch.zeros(self.N)# torch.zeros(self.N)

        for k in range(1, In.shape[0]):
            #--- Compute values of interest
            #the operation Jee_re = self.Jee * prev_r_e => triggers inplace error
            s_gaba_wie = s_gaba @ self.wie
            s_ampa_wei = s_ampa @ self.wei
            s_gaba_wii = s_gaba @ self.wii
            # ic(Sampa_Jee.grad_fn) 
            i_tot_e = torch.add(torch.subtract(torch.mul(self.Jee, s_ampa), torch.mul(self.Jie, s_gaba_wie)), torch.mul(self.Jin, In[k - 1, :]))
            ...
            #ic(prev_s_ampa.grad_fn, prev_r_e.grad_fn)
            

            prev_r_e[k,:] = r_e.clone()#.detach().clone()
            prev_r_i[k,:] = r_i.clone()#.detach().clone()
            prev_s_ampa[k,:] = s_ampa.clone()#.detach().clone()
            prev_s_gaba[k,:] = s_gaba.clone()#.detach().clone()
